...
class Agent:
    ...
    def _build_model(self):
        # Ensure the observation_shape has 3 channels
        if len(self.observation_shape) == 3 and self.observation_shape[2] == 4:
            ob_shape = (self.observation_shape[0], self.observation_shape[1], 3)

        model = Sequential()
        model.add(NASNetLarge(weights='imagenet', include_top=False, input_shape=ob_shape))
        model.add(Flatten())
...
if __name__ == "__main__":
    ...
    # Loads the weights
    if os.path.isfile("super_mario_bros2_v1_NASnet.h5"):
        agent.load_weights("super_mario_bros2_v1_NASnet.h5")
    ...
    for episode in range(num_episodes):
        ...
        if total_reward > max_reward:
            agent.save_weights("super_mario_bros2_v1_NASnet.h5")
    ...
    with open("nas_avg_game_scores.csv", "w") as f:
        for game_score in avg_game_scores:
            f.write(f"{game_score}\n")

    with open("nas_avg_steps_per_episode.csv", "w") as f:
        for steps in avg_steps_per_episode:
            f.write(f"{steps}\n")

    with open("nas_training_times.csv", "w") as f:
        for time in training_times:
            f.write(f"{time}\n")
    
    # Print average metrics over all episodes
    with open("nas_avg_metrics.csv", "w") as f:
        # column headers
